...
defmodule SubmitBlock.Geth do
  ...
  def terminate(_, container_id) when is_binary(container_id) do
    stop_container_url =
      "http+unix://%2Fvar%2Frun%2Fdocker.sock/#{@docker_engine_api}/containers/#{container_id}/stop"
  ...
  defp start_container(container_id) do
    url =
      "http+unix://%2Fvar%2Frun%2Fdocker.sock/#{@docker_engine_api}/containers/#{container_id}/start"
    ...
    case response.status_code do
      204 -> :ok
      500 -> raise ArgumentError, message: response.body
    end
  end
  ...
  defp pull_geth_image() do
    ...
    url =
      "http+unix://%2Fvar%2Frun%2Fdocker.sock/#{@docker_engine_api}/images/create?fromImage=#{
        geth_image
      }"
